    ...
    $resultado= array();
        ...
        $resultado= array();
            ...
            $resultado= array();
                ...
                $sql = "SELECT a.*, s.seccion FROM ano a  inner join seccion s on a.id_seccion=s.id where a.id > 9 ORDER by a.id asc  limit 3 ";
                $ano = mysqli_query($conexion, $sql);
                $resultado= array();
                }elseif (isset($qnto) && $qnto != null) {
                    $sql = "SELECT a.*, s.seccion FROM ano a  inner join seccion s on a.id_seccion=s.id where a.id > 12 ORDER by a.id asc  limit 3 ";
                    $ano = mysqli_query($conexion, $sql);
                    $resultado= array();
                    }elseif (isset($sexto) && $sexto != null) {
                        $sql = "SELECT a.*, s.seccion FROM ano a  inner join seccion s on a.id_seccion=s.id where a.id > 15 ORDER by a.id asc  limit 3 ";
                        $ano = mysqli_query($conexion, $sql);
                        $resultado= array();
                    }  
...
//Conseguir los años de los estudiantes
    function conseguirAnos($conexion){
        $sql = "select a.ano, a.id, s.seccion from ano a INNER join seccion s on s.id = a.id_seccion order by a.id";
        ...
        function existeNota($db, $id_alumno, $id_pensum, $lapso, $periodo){
            $sql = "select * from notas where id_pensum = '$id_pensum' and id_alumno = '$id_alumno' and lapso = $lapso and periodo = '$periodo'";
        ...
        function ConseguirNotas($conexion, $id_alumno, $pensum, $periodo){
            $sql = "select avg(n.nota) as promedio, n.lapso, m.materia from notas n inner join pensum p on p.id = n.id_pensum inner join materia m on p.id_materia = m.id where n.id_alumno = $id_alumno and n.id_pensum = $pensum and n.periodo = '$periodo' group by n.lapso, n.id_pensum";
            $guardar = mysqli_query($conexion, $sql);
            $resultado = array();
