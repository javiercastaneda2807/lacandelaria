...
//Conseguir los años de los estudiantes
    function conseguirAnos($conexion){
        $sql = "SELECT a.*, s.nombre as nombres FROM ano a  left join seccion s on a.id_seccion=s.id  ORDER by a.id asc";
        $anos = mysqli_query($conexion, $sql);
        //lo hacemos array vacio para ver si me viene con algo o no
        $resultado= array();
        
        if ($anos && mysqli_num_rows($anos)>=1) {
            $resultado = $anos;
        ...
        function Nonotas($conexion){
            $sql = "select a.nombre, a.apellido, a.cedula, n.*, p.* from notas n left join alumno a on n.id_alumno = a.id left join pensum p on n.id_pensum = p.id";
            $guardar = mysqli_query($conexion, $sql);
